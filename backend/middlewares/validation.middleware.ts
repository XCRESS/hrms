 ...
 */
export const validateRequest = (validationRules: ValidationRules) => {
  return (req: Request, res: Response, next: NextFunction): void | Response => {
    ...
    if (validationRules.enum) {
      Object.keys(validationRules.enum).forEach(field => {
        const enumValues = validationRules.enum?.[field];
        if (req.body[field] && enumValues && !enumValues.includes(req.body[field])) {
