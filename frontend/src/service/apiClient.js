...
class ApiClient {
  ...
  async resetPasswordWithToken(resetToken, newPassword) {
    return this.post('/password-reset/reset', { resetToken, newPassword });
  }
  ...
  async checkOut(payload = {}) {
    if (Array.isArray(payload)) {
      return this.post(API_ENDPOINTS.ATTENDANCE.CHECK_OUT, { tasks: payload });
    }
    const body = {
      tasks: payload.tasks || []
    };
    ['latitude', 'longitude', 'accuracy', 'capturedAt'].forEach((key) => {
      if (payload[key] !== undefined) {
        body[key] = payload[key];
      }
    });
    return this.post(API_ENDPOINTS.ATTENDANCE.CHECK_OUT, body);
  }
  ...
  async getOfficeLocations(params = {}) {
    const endpoint = buildEndpointWithQuery(API_ENDPOINTS.OFFICE_LOCATIONS.BASE, params);
    return this.get(endpoint);
  }

  async getActiveOfficeLocations() {
    return this.get(API_ENDPOINTS.OFFICE_LOCATIONS.ACTIVE);
  }

  async createOfficeLocation(data) {
    return this.post(API_ENDPOINTS.OFFICE_LOCATIONS.BASE, data);
  }

  async updateOfficeLocation(id, data) {
    return this.put(API_ENDPOINTS.OFFICE_LOCATIONS.UPDATE(id), data);
  }

  async deleteOfficeLocation(id) {
    return this.delete(API_ENDPOINTS.OFFICE_LOCATIONS.DELETE(id));
  }

  // WFH Requests
  async requestWFH(data) {
    return this.post(API_ENDPOINTS.WFH_REQUESTS.BASE, data);
  }

  async getMyWFHRequests() {
    return this.get(API_ENDPOINTS.WFH_REQUESTS.MY);
  }

  async getWFHRequests(params = {}) {
    const endpoint = buildEndpointWithQuery(API_ENDPOINTS.WFH_REQUESTS.BASE, params);
    return this.get(endpoint);
  }

  async reviewWFHRequest(id, status, reviewComment) {
    return this.patch(API_ENDPOINTS.WFH_REQUESTS.REVIEW(id), { status, reviewComment });
  }
