...
import { useMissingCheckouts } from '@/hooks/queries';
...
interface MissingCheckoutAlertProps {
  onRegularizationRequest: (data: RegularizationData) => void;
}

const MissingCheckoutAlert = ({ onRegularizationRequest }: MissingCheckoutAlertProps) => {
  const [dismissed, setDismissed] = useState(false);

  // Fetch missing checkouts using React Query - automatically refetches!
  const { data: missingCheckouts = [], isLoading: loading } = useMissingCheckouts();
  ...
  if (loading || dismissed || (missingCheckouts as Attendance[]).length === 0) {
    return null;
  }
  ...
  return (
    <div className="bg-gradient-to-r from-amber-50 to-orange-50 dark:from-amber-900/20 dark:to-orange-900/20 border border-amber-200 dark:border-amber-700 rounded-xl p-4 sm:p-6 mb-6 shadow-lg">
        ...
        <div className="flex items-start space-x-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-sm text-amber-700 dark:text-amber-300">
              You have {(missingCheckouts as Attendance[]).length} day{(missingCheckouts as Attendance[]).length > 1 ? 's' : ''} with missing checkout times
            </p>
          </div>
        </div>
        <button
      ...
      <div className="space-y-3">
        {(missingCheckouts as Attendance[]).map((attendance, index) => (
          <div
