...
interface SavedDraft {
  preLunchTasks?: string[];
  postLunchTasks?: string[];
}

const TaskReportModal = ({ isOpen, onClose, onSubmit, onSkip, isLoading, isOptional = false }: TaskReportModalProps) => {
  const [tasks, setTasks] = useState<string[]>(['']);
  const [preLunchTasks, setPreLunchTasks] = useState<string[]>(['']);
  const [postLunchTasks, setPostLunchTasks] = useState<string[]>(['']);
  ...
  // Fetch effective settings using React Query - accessible to all authenticated users
  const { data: effectiveSettings } = useEffectiveSettings(undefined, { enabled: isOpen });
  const attendanceSettings = effectiveSettings?.attendance || null;

  // Fetch today's attendance using React Query
  // useMyAttendance returns AttendanceRecord[] directly (already extracted)
  const { data: attendanceRecords = [] } = useMyAttendance({
    startDate: today,
    endDate: today,
    limit: 1
  }, { enabled: isOpen && !!user?.employeeId });
  ...
  useEffect(() => {
    // attendanceRecords is already the array of records (not wrapped)
    ...
    if (record.checkIn) {
      ...
      const now = new Date();
      const workHours = (now.getTime() - checkInDateTime.getTime()) / (1000 * 60 * 60);
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (savedTasks) {
        try {
          const parsed: SavedDraft | string[] = JSON.parse(savedTasks);
          if (parsed && Object.keys(parsed).length > 0) {
            // Check if it's structured data with pre/post lunch sections
            if (typeof parsed === 'object' && 'preLunchTasks' in parsed && 'postLunchTasks' in parsed) {
              setPreLunchTasks(parsed.preLunchTasks && parsed.preLunchTasks.length > 0 ? parsed.preLunchTasks : ['']);
              setPostLunchTasks(parsed.postLunchTasks && parsed.postLunchTasks.length > 0 ? parsed.postLunchTasks : ['']);
  ...
  const formatWorkDuration = (hours: number) => {
  ...
  const handlePreLunchTaskChange = (index: number, value: string) => {
    const newTasks = [...preLunchTasks];
    newTasks[index] = value;
    setPreLunchTasks(newTasks);
  };

  const handlePostLunchTaskChange = (index: number, value: string) => {
  ...
  const handleRemovePreLunchTask = (index: number) => {
  ...
  const handleRemovePostLunchTask = (index: number) => {
