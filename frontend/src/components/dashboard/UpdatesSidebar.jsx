            ...
            <div className="space-y-3 sm:space-y-4">
              {announcements.length > 0 ? (
                announcements.map((ann, index) => (
                  <div key={ann.id || `announcement-${index}`} className="p-2.5 sm:p-3.5 hover:bg-gray-100/70 dark:hover:bg-neutral-700/60 rounded-lg transition-colors duration-200 cursor-pointer shadow-sm border border-transparent hover:border-gray-200 dark:hover:border-neutral-700">
                    <div className="flex justify-between items-start mb-1">
                      <h4 className="font-semibold text-gray-700 dark:text-neutral-100 text-xs sm:text-sm leading-snug">{ann.title}</h4>
                    ...
                    <p className="text-[10px] sm:text-xs text-gray-600 dark:text-neutral-400 mb-2 leading-relaxed">{ann.content}</p>
                    <div className="flex justify-between items-center">
                      <p className="text-[10px] sm:text-xs text-gray-400 dark:text-neutral-500">{ann.date}</p>
                      <button className="text-cyan-600 dark:text-cyan-400 text-[10px] sm:text-xs hover:underline font-semibold">Read more</button>
                    </div>
                  </div>
                ))
              ) : (
                <div className="py-8 text-center text-gray-500 dark:text-neutral-400">No announcements</div>
              )}
            </div>
          )}
