...
interface MonthlyMetrics {
  ...
  efficiency: number;
  checkInPattern: CheckInPatternDay[];
  avgCheckInTime: number;
  pastWorkDays: number;
  recentWorkDays: MonthDay[];
}
...
const WeeklySummary = ({ attendanceReport }: WeeklySummaryProps) => {
  const monthlyMetrics = useMemo<MonthlyMetrics>(() => {
    ...
    const monthDays: MonthDay[] = thisMonthData.map(record => {
    ...
    const checkInPattern: CheckInPatternDay[] = workDays.filter(day => day.checkInTime && day.status !== 'weekend' && day.status !== 'holiday').map(day => ({
      day: day.day,
              ...
              <ResponsiveContainer width="100%" height={220}>
                <BarChart data={monthlyMetrics.checkInPattern} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                  ...
                  <YAxis
                    ...
                    tickLine={false}
                    tick={{ fontSize: 12, fill: '#6B7280' }}
                    tickFormatter={(value: number) => `${Math.floor(value)}:${String(Math.round((value % 1) * 60)).padStart(2, '0')}`}
                  />
                  <Tooltip
                    formatter={(value: number) => [`${Math.floor(value)}:${String(Math.round((value % 1) * 60)).padStart(2, '0')}`, 'Check-in Time']}
