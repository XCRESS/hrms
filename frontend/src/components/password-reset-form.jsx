...
export default function PasswordResetForm({ className, ...props }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await apiClient.requestPasswordReset(name, email);
      ...
      if (response.success) {
        ...
        toast({
          ...
          description: response.message || "Failed to submit password reset request",
          type: "error"
        });
      }
    } catch (error) {
      ...
      toast({
        title: "Request Failed",
        description: "Unable to submit password reset request. Please try again.",
        type: "error"
      });
    } finally {
      setLoading(false);
