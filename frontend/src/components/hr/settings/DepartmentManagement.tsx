...
const DepartmentManagement: React.FC<DepartmentManagementProps> = ({
  ...
  const { data: availableEmployees, isLoading: loadingEmployees } = useAvailableEmployees(
    selectedDeptForEmployees?.name,
    { enabled: !!selectedDeptForEmployees && showEmployeeModal }
  );
  ...
  const handleAssignEmployee = async (employee: DepartmentEmployee) => {
  ...
  const assignEmployee = async (employee: DepartmentEmployee) => {
    try {
      if (!selectedDeptForEmployees) return;
