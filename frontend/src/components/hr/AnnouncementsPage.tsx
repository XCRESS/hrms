...
type TargetAudience = 'all' | 'employees' | 'hr' | 'admin';
...
interface CurrentAnnouncement extends AnnouncementFormData {
  _id?: string;
  author?: string;
  authorName?: string;
}

const targetAudienceOptions: TargetAudience[] = ['all', 'employees', 'hr', 'admin'];
const statusOptions: AnnouncementStatus[] = ['draft', 'published'];
...
const AnnouncementsPage = (): JSX.Element => {
  ...
  const [currentAnnouncement, setCurrentAnnouncement] = useState<CurrentAnnouncement | null>(null);
  ...
  const { data: announcements = [], isLoading: loading, error } = useAnnouncements();
  ...
  const handleSaveAnnouncement = async (e: FormEvent<HTMLFormElement>): Promise<void> => {
    e.preventDefault();
    ...
    try {
      ...
      if (isEditing && currentAnnouncement._id) {
        result = await updateAnnouncementMutation.mutateAsync({ id: currentAnnouncement._id, ...payload });
        setMessage({ type: 'success', content: 'Announcement updated successfully!' });
      } else {
        result = await createAnnouncementMutation.mutateAsync(payload);
      ...
      if (payload.status === 'published') {
        try {
          const announcementData = result || currentAnnouncement;
  ...
  const openDeleteConfirm = (announcement: Announcement): void => {
    resetMessages();
  ...
  const confirmDeleteAnnouncement = async (): Promise<void> => {
    if (!announcementToDelete) return;
    resetMessages();
    try {
      await deleteAnnouncementMutation.mutateAsync(announcementToDelete._id);
      ...
      setMessage({
        type: 'error',
        content: error.data?.message || error.message || 'Failed to delete announcement.',
      });
    }
  };
  ...
  return (
    <div className="max-w-7xl mx-auto mt-8 p-4 md:p-6 bg-white dark:bg-slate-900 rounded-xl shadow-xl text-slate-900 dark:text-slate-50">
        ...
        <div
          className={`mb-4 p-3 rounded-md flex items-center text-sm ${
            message.type === 'success'
              ? 'bg-green-100 dark:bg-green-800/30 text-green-700 dark:text-green-200'
              : 'bg-red-100 dark:bg-red-800/30 text-red-700 dark:text-red-200'
          }`}
        >
          {message.type === 'success' ? (
      ...
      {loading && announcements.length === 0 && (
        <p className="text-center text-slate-500 dark:text-slate-400 py-10">Loading announcements...</p>
      )}
            ...
            <div key={ann._id} className="bg-slate-50 dark:bg-slate-800 p-5 rounded-lg shadow-lg relative">
              ...
              <div className="text-xs text-slate-500 dark:text-slate-500 space-y-1 md:space-y-0 md:flex md:items-center md:space-x-4 mb-3">
                <span className="flex items-center">
                  <CalendarDays size={14} className="mr-1.5 opacity-70" /> Published:{' '}
                  {new Date(ann.createdAt).toLocaleDateString()} by{' '}
                  {ann.authorName || (ann.author as unknown as { name?: string })?.name || 'System'}
                </span>
                <span className="flex items-center capitalize">
                  <Users size={14} className="mr-1.5 opacity-70" /> Audience: {ann.targetAudience}
                </span>
                {canManageAnnouncements && (
                  <span
                  ...
                  >
                    {ann.status === 'published' ? (
                      ...
                      <EyeOff size={14} className="mr-1.5 opacity-70" />
                    )}
                    Status: {ann.status}
                  </span>
                )}
              ...
              {canManageAnnouncements && (
                <div className="absolute top-4 right-4 flex space-x-2">
                  <button
                    ...
                    title="Edit"
                    className="p-1.5 text-slate-500 hover:text-cyan-600 dark:hover:text-cyan-400 transition-colors rounded-full hover:bg-slate-200 dark:hover:bg-slate-700"
                  >
                    <Edit3 size={18} />
                  </button>
                  <button
                    onClick={() => openDeleteConfirm(ann)}
                    title="Delete"
                    className="p-1.5 text-slate-500 hover:text-red-600 dark:hover:text-red-400 transition-colors rounded-full hover:bg-slate-200 dark:hover:bg-slate-700"
                  >
                    <Trash2 size={18} />
                  </button>
                </div>
            ...
            <form onSubmit={handleSaveAnnouncement} className="space-y-4">
              ...
              <div className="flex justify-end space-x-3 pt-3">
                <button
                  type="button"
                  onClick={closeModal}
                  className="px-4 py-2 text-sm font-medium text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-600 hover:bg-slate-200 dark:hover:bg-slate-500 rounded-md shadow-sm transition-colors"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  disabled={loading}
                  className="px-4 py-2 text-sm font-medium text-white bg-cyan-600 hover:bg-cyan-700 disabled:bg-cyan-400 dark:disabled:bg-cyan-700/50 rounded-md shadow-sm transition-colors"
                >
          ...
          <div className="bg-white dark:bg-slate-800 rounded-lg shadow-xl p-6 w-full max-w-sm transform transition-all">
            ...
            <div className="flex justify-end space-x-3">
              ...
              <button
                ...
                disabled={loading}
                className="px-4 py-2 text-sm font-medium text-white bg-red-600 hover:bg-red-700 disabled:bg-red-400 dark:disabled:bg-red-700/50 rounded-md shadow-sm transition-colors"
              >
