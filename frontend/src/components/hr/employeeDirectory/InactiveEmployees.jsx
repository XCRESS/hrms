...
const InactiveEmployees = () => {
  ...
  const [activatingEmployee, setActivatingEmployee] = useState(null);
  ...
  const loadInactiveEmployees = async () => {
    try {
      setLoading(true);
      const response = await apiClient.getEmployees({ status: 'inactive' });
      if (response.success || response.employees) {
        setInactiveEmployees(response.employees || []);
      }
    } catch (error) {
      ...
      toast({
        variant: "error",
        title: "Error",
        description: "Failed to load inactive employees"
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleReactivateEmployee = async (employeeId, employeeName) => {
    ...
    try {
      ...
      if (response.message) {
        ...
        setInactiveEmployees(prev => prev.filter(emp => emp._id !== employeeId));
              ...
              >
                <div className="flex flex-col lg:flex-row lg:items-center justify-between space-y-4 lg:space-y-0">
                  ...
                  <div className="flex-shrink-0 space-y-2 sm:space-y-0 sm:space-x-2 sm:flex sm:flex-row">
                    ...
                    <button
                      onClick={() => handleReactivateEmployee(employee._id, employee.fullName)}
                      disabled={activatingEmployee === employee._id}
                    ...
                    >
                      {activatingEmployee === employee._id ? (
                        <>
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="space-y-2 text-sm">
                        <div><span className="font-medium">Aadhaar Number:</span> {employeeDetails.aadhaarNumber}</div>
                        <div><span className="font-medium">PAN Number:</span> {employeeDetails.panNumber}</div>
                      </div>
                    </div>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="space-y-2 text-sm">
                        <div><span className="font-medium">Bank Name:</span> {employeeDetails.bankName}</div>
                        <div><span className="font-medium">Account Number:</span> {employeeDetails.bankAccountNumber}</div>
                        <div><span className="font-medium">IFSC Code:</span> {employeeDetails.bankIFSCCode}</div>
                        <div><span className="font-medium">Payment Mode:</span> {employeeDetails.paymentMode}</div>
                      </div>
                    </div>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="space-y-2 text-sm">
                        <div><span className="font-medium">Name:</span> {employeeDetails.emergencyContactName}</div>
                        <div><span className="font-medium">Phone:</span> {employeeDetails.emergencyContactNumber}</div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 pt-6 border-t border-gray-200 dark:border-gray-700">
                    <button
                      ...
                      disabled={activatingEmployee === employeeDetails._id}
                    ...
                    >
                      {activatingEmployee === employeeDetails._id ? (
                        <>
                          <div className="animate-spin rounded-full h-4 w-4 border-2 border-white border-t-transparent"></div>
                          <span>Reactivating...</span>
                        </>
