...
interface LeaveStats {
  ...
  thisYearLeaves: number;
  total: number;
}

// Enhanced Leave Requests Table with analytics and filtering
const LeaveRequestsTable: React.FC<LeaveRequestsTableProps> = ({ leaves, employeeProfile }) => {
  const [statusFilter, setStatusFilter] = useState<string>('all');
  const [typeFilter, setTypeFilter] = useState<string>('all');
  const [sortOrder, setSortOrder] = useState<'asc' | 'desc'>('desc');

  const calculateLeaveStats = (): LeaveStats | null => {
  ...
  const getLeaveTypeOptions = (): string[] => {
    const types = [...new Set(leaves.map(l => l.leaveType))];
    return types;
  };

  const calculateLeaveDuration = (leave: Leave): string => {
        ...
        <table className="min-w-full text-sm">
          ...
          <tbody className="bg-white dark:bg-slate-800 divide-y divide-slate-200 dark:divide-slate-700">
            {sortedLeaves.map((leave) => (
              <tr key={leave._id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                <td className="p-3 whitespace-nowrap text-slate-700 dark:text-slate-200">
                  {new Date(leave.createdAt || Date.now()).toLocaleDateString()}
                </td>
                <td className="p-3 whitespace-nowrap text-slate-700 dark:text-slate-200">
...
const LeaveSection: React.FC<LeaveSectionProps> = ({ leaves, employeeProfile }) => {
  return (
    <div className="space-y-6">
