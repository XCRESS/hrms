...
import type { SalarySlip, SalarySlipStatus, Employee, EmployeeReference } from "../../../types";

interface SalarySlipCardProps {
  slip: SalarySlip;
  employeeName: string;
  monthName: string;
  employeeData?: EmployeeReference | Employee;
  onEdit: (slip: SalarySlip) => void;
  onDelete: (slip: SalarySlip) => void;
  onPublish: (slip: SalarySlip, newStatus: SalarySlipStatus) => void;
}
...
const SalarySlipCard: React.FC<SalarySlipCardProps> = ({
  ...
  const handlePublishToggle = (): void => {
    const newStatus: SalarySlipStatus = slip.status === 'finalized' ? 'draft' : 'finalized';
